...
#include <stdint.h>
#include <inttypes.h>
//Size (in bits) of each part
#define BN_PART_SIZE 8
   ...
   #define SWAP SWAP8
   #define BN_PART_BYTES 1
#elif BN_PART_SIZE == 16
   #define l_t       int16_t
   ...
   #define SWAP SWAP16
   #define BN_PART_BYTES 2
#elif BN_PART_SIZE == 32
   #define l_t       int32_t
   ...
   #define SWAP SWAP32
   #define BN_PART_BYTES 4
#elif BN_PART_SIZE == 64
   #define l_t       int64_t
...
#define BN_PART_BITS (BN_PART_BYTES * 8)
...
/*! Convert bits to number of PARTs. */
#define BYTES_TO_PARTS(x) ((x * 8) / BN_PART_BITS + ((x * 8) % BN_PART_BITS ? 1 : 0))
#define PARTS_TO_BYTES(x) (x * BN_PART_BYTES)
...
bn_t* bn_positiv(bn_t*);
uint8_t* bn_to_u8(bn_t*);
int _bn_big_endian();
int bn_to_ptr(bn_t*,uint8_t*);
/*!
...
/*!
* \brief Allocate a new bignum of chosen size (in PARTs).
*/
bn_t *bn_alloc_parts(int size);
